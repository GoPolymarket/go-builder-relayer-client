...
const (
	...
	// Relayer error codes (RELAYER-xxx)
	CodeConfigUnsupported   ErrorCode = "RELAYER-001"
	CodeSafeDeployed        ErrorCode = "RELAYER-002"
	CodeSafeNotDeployed     ErrorCode = "RELAYER-003"
	CodeMissingGasEstimator ErrorCode = "RELAYER-004"
	CodeNoTransactions      ErrorCode = "RELAYER-005"
	CodeUnsupportedTxType   ErrorCode = "RELAYER-006"
	CodeInvalidNoncePayload ErrorCode = "RELAYER-007"
	CodeTransactionFailed   ErrorCode = "RELAYER-008"
	CodeTransactionTimeout  ErrorCode = "RELAYER-009"
...
// Relayer errors
var (
	// ErrConfigUnsupported is returned when relayer config is not supported on the chain.
	ErrConfigUnsupported = New(CodeConfigUnsupported, "config is not supported on the chainId")
	// ErrSafeDeployed is returned when the Safe is already deployed.
	ErrSafeDeployed = New(CodeSafeDeployed, "safe already deployed")
	// ErrSafeNotDeployed is returned when the Safe has not been deployed yet.
	ErrSafeNotDeployed = New(CodeSafeNotDeployed, "safe not deployed")
	// ErrMissingGasEstimator is returned when a gas estimator is required but not provided.
	ErrMissingGasEstimator = New(CodeMissingGasEstimator, "gas estimator is required")
	// ErrNoTransactions is returned when no transactions are provided for execution.
	ErrNoTransactions = New(CodeNoTransactions, "no transactions to execute")
	// ErrUnsupportedTxType is returned when a relay transaction type is unsupported.
	ErrUnsupportedTxType = New(CodeUnsupportedTxType, "unsupported relay transaction type")
	// ErrInvalidNoncePayload is returned when the nonce payload is missing or invalid.
	ErrInvalidNoncePayload = New(CodeInvalidNoncePayload, "invalid nonce payload received")
	// ErrTransactionFailed is returned when a transaction fails on-chain.
	ErrTransactionFailed = New(CodeTransactionFailed, "transaction failed onchain")
	// ErrTransactionTimeout is returned when a transaction does not reach a desired state in time.
	ErrTransactionTimeout = New(CodeTransactionTimeout, "transaction not found or not in desired state (timeout)")
)

// Backwards-compatible aliases for existing error names.
var (
	// ErrSignerUnavailable is returned when a signer is required but not provided.
	ErrSignerUnavailable = ErrMissingSigner
)
