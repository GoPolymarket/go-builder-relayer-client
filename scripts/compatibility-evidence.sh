...
Generate weekly compatibility evidence artifact for go-builder-relayer-client.
USAGE
}
...
run_check "builder_auth_remote_fallback" "go test ./... -run '^TestBuilderHeadersRemoteFallbackLocal' -count=1"
run_check "polling_contract_behavior" "go test ./... -run '^TestPollUntilState_' -count=1"
run_check "repo_compile_gate" "go test ./... -run '^$' -count=1"
